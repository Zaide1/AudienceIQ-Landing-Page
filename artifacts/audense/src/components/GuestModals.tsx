...
/* ── 3. Leaving-dashboard guard for guests with active research ──────
   Surfaced when a guest clicks the dashboard logo while they have an
   active session/map/chat. Primary opens the sign-up modal (no nav);
   secondary navigates to landing without wiping local data; closing the
   modal (overlay click via onTertiary, or X button) keeps them in place. */
export function LeavingDashboardConfirm({
  onPrimary,
  onSecondary,
  onCancel,
}: {
    ...
    <>
      ...
      <div
        style={{
          position: "fixed", top: "50%", left: "50%",
          transform: "translate(-50%,-50%)",
          zIndex: 9200, width: 420, maxWidth: "calc(100vw - 40px)",
          background: "#fff", borderRadius: 18,
          boxShadow: "0 28px 72px rgba(15,23,42,0.20)",
          padding: "32px 28px 28px",
          fontFamily: "Inter, sans-serif",
      ...
      >
        ...
        <div style={{ fontSize: 17.5, fontWeight: 700, color: "#111827", marginBottom: 10, lineHeight: 1.3 }}>
          Save this research before leaving?
        </div>
        <div style={{ fontSize: 13.5, color: "#6B7280", lineHeight: 1.6, marginBottom: 24 }}>
          You're using AudienceIQ as a guest. Sign up to save this chat, audience map, and research history before going back to the homepage.
        </div>
